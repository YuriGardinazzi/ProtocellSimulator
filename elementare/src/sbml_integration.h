...
struct SbmlModule : public BaseBiologyModule {
  ...
  void Run(SimObject* so) override {
    if (auto* cell = static_cast<MyCell*>(so)) {
      ...
      std::cout << rr_ -> getValue("A_0") << std::endl;
      if (rr_ -> getValue("A_0") < 16000){
        std::cout << rr_ -> getValue("A_0") << std::endl;
     ...
     //const auto& partial_result = rr_->getFloatingSpeciesConcentrationsNamedArray();
...
inline int Simulate(int argc, const char** argv) {
  ...
  std::string sbml_file = "../src/mv_elementare_FACILE1.xml";
  if (!FileExists(sbml_file)) {
    sbml_file = "src/mv_elementare_FACILE1.xml";
    if (!FileExists(sbml_file)) {
      Log::Error("Could not find sbml_elementare_FACILE1.xml file.");
    }
  }
