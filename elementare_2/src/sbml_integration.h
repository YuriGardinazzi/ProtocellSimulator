...
inline void PlotSbmlModules(const char* filename) {
  ...
  mg->SetTitle("FACILE2;Timestep;Concentration");
...
inline int Simulate(int argc, const char** argv) {
  ...
  std::string sbml_file = "../src/mv_elementare_FACILE2.xml";
  if (!FileExists(sbml_file)) {
    sbml_file = "src/mv_elementare_FACILE2.xml";
    if (!FileExists(sbml_file)) {
      Log::Error("Could not find mv_elementare_FACILE2.xml file.");
    }
  }
